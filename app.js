...
/* KND WAVES CORRIGIDO FINAL - importacao multipla + voltar */
(function(){
  const MAX=30, AUDIO=/\.(mp3|wav|m4a|aac|flac|ogg|opus|webm)$/i;
  const $=s=>document.querySelector(s), $$=s=>Array.from(document.querySelectorAll(s));
  function ok(f){return f&&((f.type||'').startsWith('audio/')||AUDIO.test(f.name||''));}
  function clean(n){return String(n||'Musica').replace(/\.[^/.]+$/,'').replace(/[_-]+/g,' ').trim()||'Musica';}
  function panel(){let p=$('#knd-final-panel')||$('#folder-import-panel'); if(p)return p; p=document.createElement('section'); p.id='knd-final-panel'; p.className='folder-import-panel show'; p.innerHTML='<div class="folder-import-head"><div><div class="folder-import-title" id="knd-final-title">Musicas importadas</div><div class="folder-import-sub" id="knd-final-sub">Pronto para tocar.</div></div><div class="folder-import-count" id="knd-final-count">0 de 30</div></div><div class="folder-import-list" id="knd-final-list"></div><div class="folder-import-actions"><button class="btn btn-primary" id="knd-final-play">Reproduzir tudo</button></div>'; const d=$('.upload-drop')||document.body; if(d.parentNode)d.parentNode.insertBefore(p,d.nextSibling); else document.body.appendChild(p); return p;}
  function parts(){panel();return{p:$('#knd-final-panel')||$('#folder-import-panel'),title:$('#knd-final-title')||$('#folder-import-title'),sub:$('#knd-final-sub')||$('#folder-import-sub'),count:$('#knd-final-count')||$('#folder-import-count'),list:$('#knd-final-list')||$('#folder-import-list'),play:$('#knd-final-play')||$('#folder-play-now')}}
  function render(ts,folder){const x=parts();x.p.style.display='block';x.p.classList.add('show');if(x.title)x.title.textContent=folder||'Musicas importadas';if(x.sub)x.sub.textContent=ts.length+' musica(s) importadas. Limite: '+MAX+'.';if(x.count)x.count.textContent=ts.length+' de '+MAX;if(x.list){x.list.innerHTML='';ts.forEach((t,i)=>{let item=document.createElement('div');item.className='folder-import-item';item.innerHTML='<span class="folder-import-index">'+(i+1)+'</span><span class="folder-import-name">'+t.title+'</span><span class="folder-import-meta">Local</span>';item.onclick=()=>play2(i);x.list.appendChild(item);});}if(x.play){x.play.textContent='Reproduzir tudo';x.play.onclick=()=>play2(0);}try{x.p.scrollIntoView({behavior:'smooth',block:'nearest'});}catch(e){}}
  function import2(files){let a=Array.from(files||[]).filter(ok);if(!a.length){alert('Nenhum audio compativel encontrado');return;}if(a.length>MAX){alert('Limite de 30 musicas por importacao');a=a.slice(0,MAX);}let folder=(a[0].webkitRelativePath?a[0].webkitRelativePath.split('/')[0]:(a.length>1?'Musicas selecionadas':'Musica selecionada'));let ts=a.map((f,i)=>({id:'knd_'+Date.now()+'_'+i,title:clean(f.name),project:folder,genre:'Local',description:'Importado do dispositivo',importFolder:folder,mimeType:f.type||'audio/mpeg',createdAt:Date.now()+i,_url:URL.createObjectURL(f),_file:f,_kndFinalImport:true,lrcLines:[],lyrics:''}));window.KND_IMPORTED_TRACKS=ts;window.KND_CURRENT_INDEX=0;try{if(window.S&&Array.isArray(S.tracks)){S.tracks=[...ts,...S.tracks.filter(t=>!t._kndFinalImport)];S.currentIdx=0;S.context=folder;if(typeof renderLibrary==='function')renderLibrary();if(typeof renderQueue==='function')renderQueue();if(typeof renderFeatured==='function')renderFeatured();if(typeof renderRecents==='function')renderRecents();if(typeof updateHero==='function')updateHero();}}catch(e){}render(ts,folder);try{if(typeof showPage==='function')showPage('upload');}catch(e){}}
  function audio(){return $('audio')||$('#audio')||$('#audio-player')||$('#player-audio');}
  function play2(i){let ts=window.KND_IMPORTED_TRACKS||[];if(!ts.length){alert('Nenhuma musica importada');return;}i=Math.max(0,Math.min(i||0,ts.length-1));window.KND_CURRENT_INDEX=i;let t=ts[i];try{if(window.S&&Array.isArray(S.tracks)&&typeof playTrack==='function'){let idx=S.tracks.findIndex(x=>x.id===t.id);if(idx>=0){playTrack(idx,t.importFolder||'Local');return;}}}catch(e){}let a=audio();if(!a){alert('Player nao encontrado');return;}a.src=t._url;a.play().catch(()=>alert('Toque novamente para iniciar o audio'));try{if(typeof showPage==='function')showPage('player');}catch(e){}}
  function bind(){ $$('input[type="file"]').forEach(inp=>{if(inp.dataset.kndFixed)return;inp.dataset.kndFixed='1';inp.setAttribute('multiple','multiple');inp.addEventListener('change',e=>{e.stopImmediatePropagation();e.stopPropagation();import2(inp.files);setTimeout(()=>{try{inp.value='';}catch(x){}},300);},true);}); $$('#btn-player-back,.btn-back,[data-action="back"],[aria-label="Voltar"]').forEach(b=>{if(b.dataset.kndBack)return;b.dataset.kndBack='1';b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();try{if(typeof showPage==='function'){showPage('library');return;}}catch(x){}history.back();},true);}); let a=audio();if(a&&!a.dataset.kndEnd){a.dataset.kndEnd='1';a.addEventListener('ended',()=>{let ts=window.KND_IMPORTED_TRACKS||[];if(ts.length)play2(((window.KND_CURRENT_INDEX||0)+1)%ts.length);});}}
  window.KND_IMPORT_FINAL=import2;window.KND_PLAY_IMPORTED=play2;window.addEventListener('load',bind);document.addEventListener('DOMContentLoaded',bind);setTimeout(bind,800);setTimeout(bind,1800);try{new MutationObserver(bind).observe(document.body,{childList:true,subtree:true});}catch(e){}
})();
